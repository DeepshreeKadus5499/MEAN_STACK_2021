        ...
        USER.findById(userId).then((user) => {
            if (!user || user.isCommentsBlocked) {
                return res.status(401).json({
                    message: 'Sorry, but you\'re not allowed to comment on groceries'
                });
            }
            ...
            GROCERY.findById(groceryId).then((grocery) => {
                ...
                COMMENT.create({ content: comment }).then((newComment) => {
                    grocery.comments.push(newComment._id);
                    newComment.grocery = grocery._id;
    ...
    delete: (req, res) => {
        ...
        COMMENT.findById(commentId).then((comment) => {
            ...
            COMMENT.findByIdAndRemove(comment._id).then(() => {
                GROCERY.update({ _id: comment.grocery }, { $pull: { comments: comment._id } }).then(() => {

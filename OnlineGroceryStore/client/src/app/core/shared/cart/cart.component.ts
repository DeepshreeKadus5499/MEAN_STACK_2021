...
export class CartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.cartService
      ...
      .subscribe((res) => {
        this.cart = res.data;
        this.cartForm = this.toFormGroup(this.cart.groceries);
        this.onChanges();
      });
  }
  ...
  toFormGroup(groceries: Grocery[]): FormGroup {
    const group: any = {};

    groceries.forEach(grocery => {
      group[grocery._id] = new FormControl(
        grocery.qty || '', [
          Validators.required,
          Validators.min(1),
          Validators.max(20)
  ...
  onRemove(): void {
    this.cartService
      ...
      .subscribe(() => {
        ...
        this.cart.groceries = this.cart.groceries.filter(b => b._id !== this.lastDeleteId);
  ...
  reCalcSum(formValues: object): void {
    let price = 0;
    for (const b of this.cart.groceries) {
